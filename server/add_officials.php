...
if ($preparedSql = $db->prepare("INSERT INTO `officials` (`name_officials`, `img_officials`, `position`, `direct_timestamp`) VALUES (?,?,?, ?)")) {
  ...
  if ($preparedSql->execute()) {
    ...
    $response['message'] = 'Barangay Officials had been successfully added.';
    $response['admin'] =  $_SESSION['adminLogged'];
    $response['operation'] = "add";
    $response['description'] = "Barangay Officials: <b>" . strtoupper($add_titleAnnouncements). "</b> have been added at <b>Barangay Officials.</b>";
  } else {
    $response['status'] = false;
    $response['message'] = 'Failed to add a Barangay Officials: ' . $preparedSql->error;
