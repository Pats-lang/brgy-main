...
if ($preparedSql = $db->prepare("UPDATE `officials` SET `name_officials`= ?, `img_officials`= ?, `position` = ?, `direct_timestamp` = ? WHERE id =? ")) {
    ...
    if ($preparedSql->execute()) {
        ...
        $response['message'] = 'Successfully updated Barangay Officials details.';
        $response['admin'] = $_SESSION['adminLogged'];
        $response['operation'] = "edit";
        $response['description'] = "Barangay Officials <b>" . strtoupper($edit_titleAnnouncements) . "</b> has been updated at <b>Barangay Officials.</b>";
    } else {
